...
export default function TicketSuccessScreen() {
  ...
  const { placeId, visitDate, tickets, total, bookingCode } = useLocalSearchParams<{
    placeId: string; visitDate: string; tickets: string; total: string; bookingCode: string;
  }>();
  const place = useMemo(
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#121212] px-6">
      <View className="flex-1 items-center justify-center">
        ...
        <Text className="text-white text-xl font-bold">Mua vé thành công!</Text>
        ...
        <View className="w-full bg-neutral-900 rounded-2xl p-4 mt-6">
          ...
          <View className="gap-2">
            <View className="flex-row justify-between"><Text className="text-neutral-500">Địa điểm</Text><Text className="text-white">{place.title}</Text></View>
            ...
            <View className="flex-row justify-between"><Text className="text-neutral-500">Số vé</Text><Text className="text-white">{tickets}</Text></View>
