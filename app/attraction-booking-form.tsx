import AttractionScheduleCard from "@/src/components/booking/AttractionScheduleCard";
...
import { getSchedulesByAttraction } from "@/src/services/attractionScheduleService";
import { getAttractionById } from "@/src/services/attractionService";
import { AttractionDetail } from "@/src/types/attraction";
import { AttractionSchedule } from "@/src/types/attractionSchedule";
...
export default function AttractionBookingFormScreen() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [placeData, scheduleList] = await Promise.all([
        getAttractionById(accessToken, placeId),
        getSchedulesByAttraction(accessToken, placeId),
      ]);
      setPlace(placeData);
  ...
  const unitPrice = selectedSchedule?.overridePrice ?? place.entryFee ?? 0;
  const maxGuests = selectedSchedule?.availableSlots ?? 10;
  ...
  const handleContinue = () => {
    ...
    router.push({
      pathname: "/attraction-payment",
      params: {
        placeId: place.id,
        scheduleId: selectedSchedule.id,
        visitDate: selectedSchedule.visitDate,
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#121212]">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 140 }} className="px-6">
        <Text className="text-neutral-400 text-sm mb-4">Chọn ngày phù hợp</Text>

        {schedules.length === 0 ? (
          <Text className="text-neutral-500">Chưa có lịch tham quan nào</Text>
        ) : (
          schedules.map((s) => (
            <AttractionScheduleCard
              key={s.id}
              schedule={s}
              basePrice={place.entryFee ?? 0}
          ...
          <>
            <View className="flex-row items-center justify-between bg-neutral-900 rounded-2xl p-4 mt-2">
              <View>
                ...
                <Text className="text-neutral-500 text-xs mt-1">Còn {selectedSchedule.availableSlots} vé</Text>
              </View>
              <View className="flex-row items-center gap-3">
                ...
                <TouchableOpacity onPress={() => setGuests((g) => Math.min(maxGuests, g + 1))} className="w-8 h-8 rounded-full bg-neutral-700 items-center justify-center">
            ...
            <View className="mt-4">
              ...
              <TextInput
                ...
                placeholder="Ví dụ: cần thuê xe đạp..."
            ...
            <View className="mt-6">
              <PricingSummary
                rows={[
                  { label: `$${unitPrice} × ${guests} vé`, value: `$${subtotal}` },
