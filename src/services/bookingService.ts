...
import { CreateHotelBookingRequest, CreateTourBookingRequest, HotelBookingResult, TourBookingResult } from "../types/booking";
import { HotelBookingItem, TourBookingItem } from "../types/bookingList";
import { getWithAuth, postWithAuth } from "./api";


// --- Tour ---
export async function createTourBooking(token: string | null, payload: CreateTourBookingRequest): Promise<TourBookingResult> {
...
export async function createHotelBooking(token: string | null, payload: CreateHotelBookingRequest) : Promise<HotelBookingResult> {
  const res = await postWithAuth<HotelBookingResult>("/bookings/hotels", token, payload as unknown as Record<string, unknown>);
  if(!res.data) throw new Error("Không tạo được booking khách sạn");
  return res.data;
}
