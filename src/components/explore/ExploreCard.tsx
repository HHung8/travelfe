...
const ExploreCard = ({ item, onPress }: Props) => {
  return (
    <TouchableOpacity onPress={onPress} className="bg-[#323232] rounded-3xl overflow-hidden mb-5">
      <View style={{ backgroundColor: item.color }} className="h-32 justify-center items-center">
        <Text className="text-5xl">{item.emoji}</Text>
        <View className="absolute top-4 right-4 bg-white rounded-full px-3 py-1">
          <Text className="font-semibold text-[#555]">⭐ {item.rating}</Text>
        </View>
      </View>
      <View className="p-4">
        ...
        <View className="flex-row justify-between mt-2">
          <View className="flex-row items-center">
            ...
            <Text className="text-neutral-400 ml-1">{item.location}</Text>
          </View>
          <Text className="text-violet-500 font-bold">{item.price}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
